	...
	Class fieldXML_Selectbox extends Field {
		...
		function getValuesFromXML() {
			...
			foreach($xpath->query($this->get('item_xpath')) as $item) {
				...
				if (is_null($option['value'])) $option['value'] = $option['text'];

				$options[] = $option;

				if($item->hasChildNodes()) foreach($xpath->query('*', $item) as $child) {
					$text = $this->run($xpath, $child, $this->get('text_xpath'));
					$value = $this->run($xpath, $child, $this->get('value_xpath'));

					$options[] = array(
						'text' => $option['text'] . " / " . $text,
						'value' => $option['value'] . "-" . (!is_null($value) ? $value : $text)
					);
				}
			}
		...
		function run(DOMXPath $xpath, $node, $query) {
